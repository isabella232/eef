...
import org.eclipse.emf.eef.runtime.internal.context.DelegatingPropertiesEditingContext;
	...
	private EClassBinding getEditedBinding(PropertiesEditingComponent editingComponent) {
		...
		while (context != null) {
			if (context instanceof EObjectPropertiesEditingContext) {
				if (((EObjectPropertiesEditingContext) context).getEObject() instanceof EClassBinding) {
					return (EClassBinding) ((EObjectPropertiesEditingContext) context).getEObject();
				} else {
					context = context.getParentContext();
				}
			} else if (context instanceof DelegatingPropertiesEditingContext) {
				context = ((DelegatingPropertiesEditingContext) context).getDelegatingContext();
			}
		}
		return null;
	}
